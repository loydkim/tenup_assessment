...
void main() {
  ...
  setUp(() async {
    restaurants = RestaurantQueryResult.fromJson(mockData['data']['search'])
            .restaurants ??
        [];
  });

  group('local db test', () {
    test('save and load restaurants', () async {
      await LocalDB.init(isTestMode: true);
      final localDB = LocalDB.instance;
      await localDB.saveRestaurants(restaurants);
      final savedRestaurant = await localDB.readRestaurants();
      if (savedRestaurant != null) {
        final restaurants = (savedRestaurant as List<dynamic>)
